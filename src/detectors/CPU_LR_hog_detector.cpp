...
vector<Rect> CPU_LR_HOGDetector::detect(Mat &frame)
{
	...
	//this->feature_values = MatrixXd::Zero(samples.size(),this->n_descriptors);
		for(size_t i=0;i<samples.size();i++){
			...
			//temp.normalize();				
			this->feature_values.row(i) = temp;	
		}
		VectorXd dataNorm = this->feature_values.rowwise().squaredNorm().array().sqrt();
		this->feature_values = this->feature_values.array().colwise() / dataNorm.array();
		VectorXd predict_prob = this->logistic_regression.predict(this->feature_values, true);
...
{	
	...
	unsigned int hog_channels=mat_hog_features.size();
	int cidx=0;
